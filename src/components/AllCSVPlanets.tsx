  ...
  useEffect(() => {
    const loadCSVPlanets = async () => {
      ...
      try {
        ...
        // Create some sample planets if CSV loading fails
        ...
        setFilteredPlanets(samplePlanets);
      } finally {
        setLoading(false);
      }
    };
    ...
    return (
      <div className="flex items-center justify-center py-20">
        <div className="text-center">
          ...
          <p className="text-gray-300">Loading Exoplanet Database...</p>
          <p className="text-gray-500 text-sm mt-2">Processing comprehensive planetary data...</p>
        </div>
      </div>
    );
  }
    ...
    <div>
      ...
      <div className="mb-6 space-y-4">
        ...
        <div className="flex flex-col sm:flex-row gap-4">
          <div className="relative flex-1">
            ...
            <input
              type="text"
              placeholder={`Search from ${planets.length.toLocaleString()} exoplanets...`}
              value={searchTerm}
      ...
      <div className="mb-6 text-center">
        <p className="text-gray-300">
          {searchTerm ? `Search Results for "${searchTerm}"` : 'All Exoplanets'} - 
          Showing {paginatedPlanets.length} of {filteredPlanets.length.toLocaleString()} planets
        </p>
        <p className="text-gray-500 text-sm">
          Page {currentPage} of {totalPages.toLocaleString()} • Comprehensive Database
        </p>
      </div>
      ...
      <div className="mt-8 text-center">
        <div className="inline-flex items-center gap-2 px-4 py-2 bg-green-900/20 border border-green-500/30 rounded-lg">
          ...
          <span className="text-green-300 text-sm">
            Comprehensive Exoplanet Database - {planets.length.toLocaleString()} Confirmed Planets
          </span>
        </div>
      </div>
    </div>
